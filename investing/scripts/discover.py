...
def rewrite_watchlist_text(old_text: str, rows: list[str], today: dt.date) -> str:
    ...
    block = [
        ...
        "# These are LEADS (mechanical pass only), never buy candidates. To promote one:",
        "# write setups/<ticker>.md AND screen it compliant in Zoya/Musaffa, then run",
        "# recommend.py. Re-run discovery to refresh. Shariah here is UNVERIFIED.",
